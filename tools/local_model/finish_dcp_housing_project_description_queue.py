#!/usr/bin/env python3
"""Run DCP housing-project descriptions after earlier local-model passes."""
import argparse
import os
import subprocess
import sys
...
def main():
    ...
    stem = "dcp-housing-project-description"
    database = work / f"{stem}-units.db"
    run(python, root / "tools/merges/mine_dcp_housing_project_description_units.py",
        "--db", database)
    run(python, root / "tools/local_model/export_dob_description_packets.py",
        ...
        "--progress-source", "dcp_housing_project_descriptions",
        "--dataset", "br6q-ssj3", "--id-field", "job_number",
        "--packet-prefix", stem, "--source-type", "dcp_housing_project_description")
    ...
    run(python, root / "tools/merges/merge_dob_description_results.py",
        ...
        "--source", "dcp_housing_project_description_units",
        "--methodology", "Explicit apartment label in an official DCP housing-project description; deterministically parsed, reviewed by local Qwen, and retained with verbatim evidence",
        "--match-rationale", "The DCP project record supplies the unit label and a direct NYC BBL",
        "--apply")
