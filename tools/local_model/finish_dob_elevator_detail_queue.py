#!/usr/bin/env python3
"""Run DOB elevator-detail descriptions after earlier local-model passes."""
import argparse
import os
import subprocess
import sys
...
def main():
    ...
    stem = "dob-elevator-detail"
    database = work / f"{stem}-units.db"
    run(python, root / "tools/merges/mine_dob_elevator_detail_units.py",
        "--db", database, "--catalog-db", catalog)
    run(python, root / "tools/local_model/export_dob_description_packets.py",
        ...
        "--progress-source", "dob_elevator_detail_descriptions",
        "--dataset", "juyv-2jek", "--id-field", "device_id",
        "--packet-prefix", stem, "--source-type", "dob_elevator_device_description")
    ...
    run(python, root / "tools/merges/merge_dob_description_results.py",
        ...
        "--source", "dob_elevator_device_description_units",
        "--methodology", "Explicit apartment label in an official DOB elevator-device description; deterministically parsed, reviewed by local Qwen, and retained with verbatim evidence",
        "--match-rationale", "The DOB device address resolves to one exact NYC PAD building address",
        "--apply")
