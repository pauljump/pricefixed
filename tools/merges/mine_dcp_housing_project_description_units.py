#!/usr/bin/env python3
"""Collect apartment labels from official DCP housing-project descriptions."""
import argparse
import json
import sqlite3
import sys
...
WHERE = (
    "bbl is not null AND (upper(job_desc) like '%APT %' OR "
    "upper(job_desc) like '%APT.%' OR upper(job_desc) like '%APT#%' OR "
    "upper(job_desc) like '%APARTMENT%')"
)
...
def query(offset, limit):
    fields = "job_number,bbl,addressnum,addressst,job_desc,datefiled"
    params = urlencode({
        "$select": fields, "$where": WHERE, "$order": "job_number",
...
def main():
    ...
    """)
    source = "dcp_housing_project_descriptions"
    state = connection.execute(
        "SELECT offset,complete FROM progress WHERE source=?", (source,)
    ).fetchone()
    offset = state[0] if state else 0
    if state and state[1]:
        print("DCP housing-project description mining already complete")
        connection.close()
        return
    pages = 0
    while True:
        ...
        output = []
        for row in rows:
            description = str(row.get("job_desc") or "").strip()
            labels = extract_explicit_unit_labels(description)
            address = " ".join(
                str(row.get(field) or "").strip().upper()
                for field in ("addressnum", "addressst") if row.get(field)
            )
            output.append((
                str(row.get("job_number") or ""), normalize_bbl(row.get("bbl")),
                address, "", description, observed_date(row.get("datefiled")),
